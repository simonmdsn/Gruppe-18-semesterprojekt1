 ...
 * @author Lukas
 */
public class FXMLIdahoMapController implements Initializable {
    @FXML
    ...
    @FXML
    private void handleIdahNorthButton(ActionEvent event) {
        mapLabel.setText("You can't go in that direction!");

    }

    @FXML
    private void handleIdahEastButton(ActionEvent event) {
        mapLabel.setText("You can't go in that direction!");
    ...
    private void handleIdahPlayButton(ActionEvent event) throws IOException {
        if(geosimExtensions.Questions.idaho == 0) {
